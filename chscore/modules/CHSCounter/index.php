...
class CHSCounterAdmin
{
 ...
 /**
  * Instance of the {@link CHSLanguage} module.
  *
 ...
 /**
  * Detects valid user action, prepares password hashes and language module.
  */
 function __construct()
 {
  ...
  if(!$this->chsLanguage->setModule('CHSCounter')) //Set shortcut
   trigger_error('CHSCounterAdmin::__construct(): Cannot set module name as shortcut', E_USER_WARNING);
...
# Login #
   ...
   if(isset($_POST['countpw']))
   {
    ...
    if(CHSFunctions::getHash($_POST['countpw']) == $this->curPassHash)
    {
     ...
     exit($this->chsLanguage->getString('logged_in', 'login') . ' <a href="' . $_SERVER['PHP_SELF'] . '?action=admin">' . $this->chsLanguage->getString('go_on', 'common') . '</a>');
    }
   }
...
?>
  ...
  <?=$this->chsLanguage->getString('enter_pass', 'login')?> <input type="password" name="countpw" <?php if(isset($_POST['countpw'])) echo('style="border-color:#FF0000;" '); ?>/> <?=CHSFunctions::getFont(1)?>(<?=$this->chsLanguage->getString('forgotten', 'login')?> <a href="<?=$_SERVER['PHP_SELF']?>?action=newpass"><?=$this->chsLanguage->getString('request_new_pass', 'login')?></a>)</span><br />
   ...
   if(isset($_POST['update']))
   {
    ...
    else
    {
     ...
     if(!empty($_POST['countpw']))
     {
      ...
      CHSFunctions::setPHPDataFile($this->curPassFile = md5(time()) . '.dat', $_SESSION['countpw']);
     }
     //New counter value
...
?></select></td></tr>
   ...
   <tr><td rowspan="2"><?=$this->chsLanguage->getString('output_counter', 'admin')?></td><td><input type="radio" name="img" value="true"<?php if($settings['img']) echo(' checked="checked"'); ?> /><?=$this->chsLanguage->getString('as_pic', 'admin')?></td></tr>
  ...
  <input type="submit" value="<?=$this->chsLanguage->getString('update_now', 'admin')?>" /> <input type="reset" /> <input type="button" value="<?=$this->chsLanguage->getString('title', 'logout')?>" onclick="document.location='<?=$_SERVER['PHP_SELF']?>?action=logout';" />
...
# Installation #
   ...
   if($this->action == 'install')
   {
    ...
    else
    {
     ...
     CHSFunctions::setPHPDataFile(md5(time()) . '.dat', CHSFunctions::getHash($_POST['countpw']));
...
?>
  ...
  <p><a href="http://<?=$_SERVER['SERVER_NAME']?>/"><?=$this->chsLanguage->getString('goto1', 'install')?></a> &ndash; <a href="<?=$_SERVER['PHP_SELF']?>"><?=$this->chsLanguage->getString('goto2', 'install')?></a></p>
...
$counterACP = new CHSCounterAdmin;
$counterACP->execute();
?>